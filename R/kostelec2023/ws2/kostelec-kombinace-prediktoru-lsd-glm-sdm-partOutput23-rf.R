...
path.igaD <- "D:/PersonalWork/Balej/sdmKostelec2023/k2023-23-rf/"
...
m <- sdm::sdm(as.formula(paste0(species.parts.f, "~", pred.names.all.f)), data = d, methods = c("rf"), replication = c("cv"), cv.folds = 3, n = 5, seed = TRUE)
...
write.table(df.merge.train, file = paste0(path.igaD, "rf-23-train.csv"), append = TRUE, quote = TRUE, sep = ",", row.names = FALSE, col.names = TRUE)
write.table(df.merge.test, file = paste0(path.igaD, "rf-23-test.csv"), append = TRUE, quote = TRUE, sep = ",", row.names = FALSE, col.names = TRUE)
