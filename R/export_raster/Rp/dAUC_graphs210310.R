...
# gather("Training.AUC", "Training.AUC.all", key = "Question", value = "dAUC")



# appendix <- "AUC"
# appendix <- "AUC_newOrder"
# appendix <- "dAUC"
appendix <- "dAUC_newOrder"
...
# # labels
# titfreqpixel_size <- "Dependence of dAUC on species frequency level (according to grain)"
...
# labelfreq <- "species frequency level (1 = most frequent species)"
# labely <- "AUC difference"

# tithab <- "Dependence of dAUC on habitat and grain"
# labelhab <- "Habitat type: A - farmland, open sites, F - forests, trees, U - urban, W - wetland"

# titprot <- "Dependence of dAUC on protection statut and grain"
# labelprot <- "Protection statut: N—non-protected, E—endangered, H — highly endangered, C — critically endangered"

# titdist <- "Dependence of dAUC on European distribution and grain"
# labeldist <- "European distribution: C — central, N — northern, S — southern, W — widespread"


# pdf(paste0("l_30-20-no-", appendix, ".pdf"))


# #----------------------------------------------------------------------------------------------------------------
# # check if there is a dependence of AUC diff on species frequency level
# pic_lm <- ggplot(df, aes(x = recnum_n, y = as.numeric(dAUC))) +
#   geom_point() +
#   geom_smooth(method = loess) +
#   facet_grid(Question ~ pixel_size) +
#   labs(title = titfreqpixel_size, subtitle = titQ1Q2, x = labelfreq, y = labely)
# print(pic_lm)
