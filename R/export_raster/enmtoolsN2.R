...
replicates <- 3 # 4 pro checkerboard2 (4foldy)
pref <- "_ndopVal_" # předpona png obrázků s predikcí a dalších outputů / OWNPFr /// _OF-ps80_ BFit
...
for (px_size_item in px_size) {
    ...
    for (sp in species) { # sp in ptaci_gbif_distinct$species
        ...
        if (eval == FALSE) {
            test.prop.fit <- test.prop # pro jistotu manuální přepis proměnné, nasazení podílu testovacích nálezů se totiž mění při jednotlivých adjustech -> nestabilní
            ...
            for (s in intervals) { # seq(0.05, 10.05, by = by)
                ...
                if (s == 0) {
                    ...
                    fm[["0.00"]] <- fit_modelsN2(alg, replicates.fit, eval, test.prop.fit, enm_mxt_gbif.s, enm_mxt_ndop.s, NA, raster_stack_b, raster_stack_mask_czechia_b, bias_gbif1, bias_ndop, bias_all, nback_all, nback_ndop, FALSE, fit_gbif_crop, czechia_3035)
                } else {
                    bias_gbif <- raster(paste0(export_path, "/inputs/bias_rasters2/Xbias_gbif-density-", bv, "-", px_size_item, ".tif"))
                    fm[[bvn]] <- fit_modelsN2(alg, replicates.fit, eval, test.prop.fit, enm_mxt_gbif.s, enm_mxt_ndop.s, NA, raster_stack_b, raster_stack_mask_czechia_b, bias_gbif, bias_ndop, bias_all, nback_all, nback_ndop, FALSE, fit_gbif_crop, czechia_3035)
                }
            }
            ...
            for (i in names(fm)) {
                ...
                enm_mxt_gbif <- fm[[i]]$enm_mxt_gbif
                ...
                #     writeRaster(enm_mxt_gbif.r.m, paste0(export_path, "/outputs/r/", pres, "_", sp, "_", px_size_item, "_", replicates, "_gbif---",as.character(i),".tif"), format = "GTiff", overwrite = TRUE)
                # }
                png(paste0(export_path, "/outputs/png/", sp, "_", px_size_item, "_", pres, "_", replicates, "_gbif-", as.character(i), ".png"), width = 1400, height = 1000)
                plot(enm_mxt_gbif.r.m,
                    main = paste0(sp, " | GBIF, AUC=", round(enm_mxt_gbif.auc, digits = 2), " (", (px_size_item / 1000), "km)"),
                    sub = paste0("GBIF: ", gbif_f_n.f, " | adj: ", as.character(i))
                )
                par(bg = NA)
                ...
                par()
                points(enm_mxt_ndop.pp.orig, col = rgb(red = 1, green = 0, blue = 0, alpha = 0.5), pch = 16, cex = 0.5)
                dev.off()

                enmsr[[as.character(px_size_item)]][[as.character(sp)]][[as.character(i)]] <- list(
                    enm_mxt_gbif.perf = enm_mxt_gbif.perf,
                    enm_mxt_gbif.auc = enm_mxt_gbif.auc,
                    enm_mxt_gbif.np.tr = enm_mxt_gbif.np.tr,
                    enm_mxt_gbif.np.te = enm_mxt_gbif.np.te
                )

                auc_table[[i]] <- enm_mxt_gbif.auc
            }
            print("získání nej smoothingu")

            auc_table.a <- as_tibble(auc_table)
            auc_table.b <- as_tibble(cbind(nms = names(auc_table.a), t(auc_table.a))) %>%
