...
permImp_comb2 <- function(species.selected, species.selected.ndop, env.sentinel_bio, path.igaD, replicates, k = 0) {
  data <- list()
  ...
  for (pc in all) {
    ...
    data[[pc.name]]$species.ndop.p <- NA
    ...
    thr <- list()
    # bc <- list()
    for (adj in c(0)) { # ideálně 0:9 / c(0, 1, 3, 5, 7, 9)
      bias_czechia <- raster(paste0(path.igaD, "bias-ptaci-adj-0.", as.character(adj), "-kfme16.tif"))
    ...
    dir.create(paste0(path.igaD, "predAUCglmLSD"), showWarnings = FALSE)
    ...
    writeRaster(rr, paste0(path.igaD, "predAUCglmLSD/", data[[pc.name]]$species, "-auc", format(round(ba[[adj.selected]], 2), nsmall = 2), "---", pc.name, ".tif"), format = "GTiff", overwrite = TRUE)
    ...
    # vytvoří adresář pro export rasterů prediktorů, pokud neexistuje
    dir.create(paste0(path.igaD, "predSorensenGlmLSD"), showWarnings = FALSE)

    rr2 <- stack(sapply(m2, function(x) x$suitability))
    rr2 <- calc(rr2, fun = median)
    # bacha, může toho být hodně v rámci fittování všech kombinací prediktorů a druhů!
    writeRaster(rr2, paste0(path.igaD, "predSorensenGlmLSD/", data[[pc.name]]$species, "-sor", format(round(bs[[adj.selected.sorensen]], 2), nsmall = 2), "---", pc.name, ".tif"), format = "GTiff", overwrite = TRUE)
...
# všechny kombinace prediktorů + bias fitting
permImp_comb <- function(species.selected, species.selected.ndop, env.sentinel_bio, path.igaD, replicates, k = 0, combs, r.pa) {
  data <- list()
