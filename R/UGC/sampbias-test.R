...
required_packages <-
    c("raster", "tidyverse", "sf", "sp", "lubridate", "magrittr", "dplyr", "spatialEco", "dismo", "ENMToolsPB", "spatstat", "purrr", "abind", "sampbias", "maptools") # "rmaxent", "blockCV", "ggplot2", "MASS", "data.table", "virtualspecies" (convertToPA - problematické definovat parametry v reálném světě...)
...
alg <- "glm" # "glm" "maxent"
px_size <- c(2000) # 100, 500, 1000, 5000, 10000 # 10000, 5000, 1000, 500, 100
replicates <- 1 # 4 pro checkerboard2 (4foldy)
pres <- "_OF-ps80_" # předpona png obrázků s predikcí a dalších outputů / OWNPFr
...
for (px_size_item in px_size) {
    ...
    if (use_bias == TRUE) {
        if (generate_bias_raster == TRUE) {
            ...
            bias_gbif <- resample(raster(density.ppp(gbif_ppp, sigma = px_size_item * 8)), raster_stack[[1]], method = "bilinear") # sigma = bw.scott.iso(gbif_ppp), adjust=0.5)
            ...
            # ppp <- rescale(ppp, 10, "km")
            bias_all <- resample(raster(density.ppp(all_ppp, sigma = px_size_item * 8)), raster_stack[[1]], method = "bilinear")
            ...
            # ppp <- rescale(ppp, 10, "km")
            bias_ndop <- resample(raster(density.ppp(ndop_ppp, sigma = px_size_item * 8)), raster_stack_mask_czechia[[1]], method = "bilinear")
