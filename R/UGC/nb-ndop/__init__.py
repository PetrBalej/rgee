...
# filter_page = s.post(SEARCH_URL, data=search_payload)
    ...
    table_payload = {
        ...
        #'meziexport_sloupec_CXAKCE_AUTOR': 0,
        'meziexport_sloupec_CXAKCE_DATI_DO': 1,
        'meziexport_sloupec_CXAKCE_DATI_OD': 1,
        'meziexport_sloupec_CXAKCE_ZDROJ': 1,
        #'meziexport_sloupec_CXEVD': 0,
        #'meziexport_sloupec_CXKATASTR_NAZEV': 0,
        #'meziexport_sloupec_CXLOKAL_ID': 0,
        'meziexport_sloupec_CXLOKAL_KVADRAT_XY': 1,
        #'meziexport_sloupec_CXLOKAL_NAZEV': 0,
        #'meziexport_sloupec_CXLOKAL_POZN': 0,
        ...
        #'meziexport_sloupec_CXREDLIST': 0,
        'meziexport_sloupec_CXTAXON_IDX_CATEG': 1,
        'meziexport_sloupec_CXTAXON_NAME': 1,
        'meziexport_sloupec_CXVALIDACE': 1,
        #'meziexport_sloupec_CXVYHL': 0,
        'meziexport_sloupec_HOD_VEROH': 1,
        'meziexport_sloupec_ID_ND_NALEZ': 1,
        'meziexport_sloupec_NEGATIVNI': 1,
        #'meziexport_sloupec_ODHAD': 0,
        #'meziexport_sloupec_POCET': 0,
        #'meziexport_sloupec_POKRYVNOST': 0,
        #'meziexport_sloupec_POP_POC': 0,
        #'meziexport_sloupec_POZ_HAB': 0,
        #'meziexport_sloupec_POZ_HABLOK': 0,
        #'meziexport_sloupec_POZNAMKA': 0,
        #'meziexport_sloupec_REL_POC': 0,
        #'meziexport_sloupec_STRUKT_POZN': 0,
        #'meziexport_sloupec_TAX_NOTE': 0,
...
def get_ndop_csv_data(s,num_rec,table_payload,output_name):
    ...
    for i in range(0, num_rec, 1000):
        ...
        table_url = (
            ...
            '&orderhow=DESC&frompage={frompage}&pagesize=1000&'
        ...
        if req.headers['Content-Type'] != "text/csv;charset=UTF-8" and req.text[0:5] != "PORADI":
            print(req.headers['Content-Type']+" 1(10) ******************************************************************************************************")
            time.sleep(10)
        ...
        if req.headers['Content-Type'] != "text/csv;charset=UTF-8" and req.text[0:5] != "PORADI":
            print(req.headers['Content-Type']+" 2(60) ******************************************************************************************************")
            time.sleep(60)
            req = s.post(url=table_url, data=table_payload)
            req.encoding = 'cp1250'
        ...
        if req.headers['Content-Type'] != "text/csv;charset=UTF-8" and req.text[0:5] != "PORADI":
            ...
            print(to)
            print(counter)
            print("partial CSV table downloaded: {}_tab-err-.csv".format(output_name))
            sys.exit(1)
